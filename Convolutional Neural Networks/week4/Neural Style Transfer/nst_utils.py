...
def reshape_and_normalize_image(image):
    ...
    """
    # Save image's shape
    global ori_image_shape
    ori_image_shape = image.shape[0:2]
    
    # Resize image to CONFIG HEIGHT, WIDTH, CHANNELS
...
def save_image(path, image):
    ...
    # Return original image's shape
    image = cv2.resize(image, dsize=(ori_image_shape[1], ori_image_shape[0]), interpolation=cv2.INTER_CUBIC)
